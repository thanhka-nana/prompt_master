...
class QuanLyLuuTuDong:
    def __init__(self, view, model_quan_ly_tab):
        self.view = view
    ...
    def _thuc_hien_luu(self, tab_view):
        ...
        self.model.luu_tab(tab_view.model.id, noi_dung, con_tro, cuon)
        self.view.cap_nhat_trang_thai(f"Đã lưu tab {tab_view.model.tieu_de} lúc {self._lay_gio_hien_tai()}")

    def _khi_chuyen_tab(self, event):
        # Lưu tab hiện tại ngay lập tức khi chuyển sang tab khác
        # (Thực tế nên lưu tab cũ, nhưng đơn giản là lưu tab đang active trước khi đổi hẳn)
        tid, tab_view = self.view.notebook.lay_tab_active()
        if tab_view:
            self._thuc_hien_luu(tab_view)

    def _khi_dong_app(self):
        # Lưu tất cả các tab trước khi thoát
        for tid, tab_view in self.view.notebook.tabs_dict.items():
            self._thuc_hien_luu(tab_view)
        self.view.destroy()

    def _lay_gio_hien_tai(self):
